...
void Visualizor2D::ShowImageWithDetectedFeatures(const std::string &window_title, const GrayImage &image, const std::vector<Vec2> &pixel_uv, RgbPixel color) {
    RgbImage show_image;
    DrawImageWithDetectedFeatures(image, pixel_uv, color, show_image);
    Visualizor2D::ShowImage(window_title, show_image);
}

void Visualizor2D::DrawImageWithDetectedFeatures(const GrayImage &image, const std::vector<Vec2> &pixel_uv, RgbPixel color, RgbImage &show_image) {
...
void Visualizor2D::ShowImageWithTrackedFeatures(const std::string &window_title, const GrayImage &cur_image, const std::vector<Vec2> &ref_pixel_uv,
                                                const std::vector<Vec2> &cur_pixel_uv, const std::vector<uint8_t> &track_status,
                                                uint8_t min_valid_track_status_value, RgbPixel tracked_color, RgbPixel untracked_color,
                                                RgbPixel flow_line_color) {
    RgbImage show_image;
    DrawImageWithTrackedFeatures(cur_image, ref_pixel_uv, cur_pixel_uv, track_status, min_valid_track_status_value, tracked_color, untracked_color,
                                 flow_line_color, show_image);
    Visualizor2D::ShowImage(window_title, show_image);
}

void Visualizor2D::DrawImageWithTrackedFeatures(const GrayImage &cur_image, const std::vector<Vec2> &ref_pixel_uv, const std::vector<Vec2> &cur_pixel_uv,
    ...
    for (uint32_t i = 0; i < ref_pixel_uv.size(); ++i) {
        ...
        ImagePainter::DrawBressenhanLine(show_image, ref_pixel_uv[i].x(), ref_pixel_uv[i].y(), cur_pixel_uv[i].x(), cur_pixel_uv[i].y(), flow_line_color);
    }
}

void Visualizor2D::ShowImageWithTrackedFeatures(const std::string &window_title, const GrayImage &ref_image, const GrayImage &cur_image,
                                                const std::vector<Vec2> &ref_pixel_uv, const std::vector<Vec2> &cur_pixel_uv,
                                                const std::vector<uint8_t> &track_status, uint8_t min_valid_track_status_value, RgbPixel tracked_color,
                                                RgbPixel untracked_color) {
    RgbImage show_image;
    DrawImageWithTrackedFeatures(ref_image, cur_image, ref_pixel_uv, cur_pixel_uv, track_status, min_valid_track_status_value, tracked_color, untracked_color,
                                 show_image);
    Visualizor2D::ShowImage(window_title, show_image);
}

void Visualizor2D::DrawImageWithTrackedFeatures(const GrayImage &ref_image, const GrayImage &cur_image, const std::vector<Vec2> &ref_pixel_uv,
                                                const std::vector<Vec2> &cur_pixel_uv, const std::vector<uint8_t> &track_status,
...
void Visualizor2D::ShowImageWithTrackedFeaturesWithId(const std::string &window_title, const GrayImage &ref_image, const GrayImage &cur_image,
                                                      const std::vector<Vec2> &ref_pixel_uv, const std::vector<Vec2> &cur_pixel_uv,
                                                      const std::vector<uint32_t> &ref_ids, const std::vector<uint32_t> &cur_ids,
                                                      const std::vector<uint8_t> &tracked_status, uint8_t min_valid_track_status_value,
                                                      const std::vector<uint32_t> &ref_tracked_cnt, const std::vector<Vec2> &cur_optical_velocity) {
    RgbImage show_image;
    DrawImageWithTrackedFeaturesWithId(ref_image, cur_image, ref_pixel_uv, cur_pixel_uv, ref_ids, cur_ids, tracked_status, min_valid_track_status_value,
                                       ref_tracked_cnt, cur_optical_velocity, show_image);

    Visualizor2D::ShowImage(window_title, show_image);
}

void Visualizor2D::DrawImageWithTrackedFeaturesWithId(const GrayImage &ref_image, const GrayImage &cur_image, const std::vector<Vec2> &ref_pixel_uv,
    ...
    Visualizor2D::DrawFeaturesWithIdByTrackedNumbers(ref_pixel_uv, ref_ids, Pixel(0, 0), empty_tracked_status, min_valid_track_status_value, ref_tracked_cnt,
                                                     show_image);

    // [right] Draw points in current image.
    Visualizor2D::DrawFeaturesWithIdByOpticalVelocity(cur_pixel_uv, cur_ids, Pixel(cur_image.cols(), 0), tracked_status, min_valid_track_status_value,
                                                      cur_optical_velocity, show_image);
}

void Visualizor2D::ShowImageWithTrackedFeaturesWithId(const std::string &window_title, const GrayImage &ref_image_left, const GrayImage &ref_image_right,
                                                      ...
                                                      const std::vector<uint8_t> &cur_stereo_tracked_status, uint8_t min_valid_track_status_value,
                                                      const std::vector<uint32_t> &ref_tracked_cnt, const std::vector<Vec2> &cur_optical_velocity) {
    RgbImage show_image;
    DrawImageWithTrackedFeaturesWithId(ref_image_left, ref_image_right, cur_image_left, cur_image_right, ref_pixel_uv_left, ref_pixel_uv_right,
                                       cur_pixel_uv_left, cur_pixel_uv_right, ref_ids_left, ref_ids_right, cur_ids_left, cur_ids_right, frame_tracked_status,
                                       ref_stereo_tracked_status, cur_stereo_tracked_status, min_valid_track_status_value, ref_tracked_cnt,
                                       cur_optical_velocity, show_image);

    Visualizor2D::ShowImage(window_title, show_image);
}

void Visualizor2D::DrawImageWithTrackedFeaturesWithId(
    const GrayImage &ref_image_left, const GrayImage &ref_image_right, const GrayImage &cur_image_left, const GrayImage &cur_image_right,
    const std::vector<Vec2> &ref_pixel_uv_left, const std::vector<Vec2> &ref_pixel_uv_right, const std::vector<Vec2> &cur_pixel_uv_left,
    const std::vector<Vec2> &cur_pixel_uv_right, const std::vector<uint32_t> &ref_ids_left, const std::vector<uint32_t> &ref_ids_right,
    const std::vector<uint32_t> &cur_ids_left, const std::vector<uint32_t> &cur_ids_right, const std::vector<uint8_t> &frame_tracked_status,
    const std::vector<uint8_t> &ref_stereo_tracked_status, const std::vector<uint8_t> &cur_stereo_tracked_status, uint8_t min_valid_track_status_value,
    const std::vector<uint32_t> &ref_tracked_cnt, const std::vector<Vec2> &cur_optical_velocity, RgbImage &show_image) {
    ...
    Visualizor2D::DrawFeaturesWithIdByTrackedNumbers(ref_pixel_uv_left, ref_ids_left, ref_left_offset, status, min_valid_track_status_value, ref_tracked_cnt,
                                                     show_image);

    // [top right] Draw points in reference right image.
    Visualizor2D::DrawFeaturesWithIdByTrackedNumbers(ref_pixel_uv_right, ref_ids_right, ref_right_offset, ref_stereo_tracked_status,
                                                     min_valid_track_status_value, ref_tracked_cnt, show_image);

    // [bottom left] Draw points in current left image.
    Visualizor2D::DrawFeaturesWithIdByOpticalVelocity(cur_pixel_uv_left, cur_ids_left, cur_left_offset, frame_tracked_status, min_valid_track_status_value,
                                                      cur_optical_velocity, show_image);

    // [bottom right] Draw points in current right image.
    Visualizor2D::DrawFeaturesWithIdByOpticalVelocity(cur_pixel_uv_right, cur_ids_right, cur_right_offset, cur_stereo_tracked_status,
                                                      min_valid_track_status_value, cur_optical_velocity, show_image);
}

void Visualizor2D::DrawFeaturesWithIdByTrackedNumbers(const std::vector<Vec2> &pixel_uv, const std::vector<uint32_t> &ids, const Pixel &pixel_offset,
...
void Visualizor2D::DrawFeaturesWithIdByOpticalVelocity(const std::vector<Vec2> &pixel_uv, const std::vector<uint32_t> &ids, const Pixel &pixel_offset,
                                                       const std::vector<uint8_t> &status, const uint8_t min_valid_status_value,
